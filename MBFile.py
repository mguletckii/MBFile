...
def mb_get_package(addr, fun, offset, quantity, write_data=None):
    ...
    if write_data is not None:
        ...
        for byte in write_data.to_bytes(2, byteorder='big'):
            data.append(byte)
...
def mb_check_error(config, data):
    ...
    if crc == data[-2:]:
            ...
            if data[1] in error:
                ...
                if data[1] == 1:
                    print('Wrong data!', config["host"], ':', config["port"], 'Addr:',
                          config["address"], 'Command quantity: ')
                else:
                    return True
        else:
        ...
        print('Modbus CRC Error!', config["host"], ':', config["port"], 'Addr:',
              config["address"], 'Received CRC: ', data[-2:], 'CRC: ', crc, 'Data: ', data)
    return False
            ...
            for trying in range(int(config["try"])):
                try:
                    sock.send(mb_get_package(int(device["address"]), int(device["function"]),
                                             int(device["offset"]), int(device["quantity"]))) #Отправка запроса
                    data_tx = sock.recv(int(device["quantity"]) * 2 + 5) #Получение данных из сокета
                    if mb_check_error(config, device, data_tx): #Проверка контрольной суммы
                        print("Done", device)
                        data_list.update(mb_data_to_dict(device, data_tx)) #Запись данные в массив
...
def mb_read_data(sock, config, cmd, offset, quantity):
    import time
    try:

        for trying in range(int(config["try"])):
            try:
                sock.send(mb_get_package(int(config["address"]), cmd, offset, quantity))  # Отправка запроса
                data_tx = sock.recv(quantity * 2 + 5)  # Получение данных из сокета
                if mb_check_error(config, data_tx):  # Проверка контрольной суммы
                    return data_tx[3:-2]
                    break
                print("Try again")
                ...
                print('Modbus Timeout. Time: ', config["timeout"], ' msec!', config["host"],
                      ':', config["port"], 'Addr:', config["address"])
            except KeyError:
                print("Modbus Config ERROR!", config["host"], ":", config["port"])
                break
...
def mb_write_data(sock, config, cmd, offset, quantity, data):
    import time
    try:

        for trying in range(int(config["try"])):
            try:
                sock.send(mb_get_package(int(config["address"]), cmd, offset, quantity, data))  # Отправка запроса
                data_tx = sock.recv(8)  # Получение данных из сокета
                if mb_check_error(config, data_tx):  # Проверка контрольной суммы
                    return 0
                    break
                print("Try again")
...
def mb_get_file(sock, config):
    import os
    import sys

    try:
        select = int.from_bytes(mb_read_data(sock, config, 3, int(config["file_select_reg"]), 1), byteorder='big')
        weight = int.from_bytes(mb_read_data(sock, config, 3, int(config["file_weight_reg"]), 1), byteorder='big')

        directory = programm_path() + config["file_folder"]

        if not os.path.exists(directory):
            os.makedirs(directory)

        try:
            file_weight = os.path.getsize(programm_path() + config["file_folder"] + "/" + str(select))
        except Exception:
            file_weight = None

        if file_weight != weight*2:

            print("New file!", "Weight:", file_weight, ":", weight*2)

            f = open(programm_path() + config["file_folder"] + "/" +str(select), 'wb')
            sys.stdout.write('[')
            progress_index = 0
            for offset in range(int(config["file_start_reg"]), int(config["file_start_reg"]) + weight, 125):
                if (int(config["file_start_reg"]) + weight) - offset >=125:

                    f.write(mb_read_data(sock, config, 3, offset, 125))

                    for i in range(int((((offset + 125 - int(config["file_start_reg"]))/weight)*100)//5) - progress_index):
                        sys.stdout.write('=')
                        progress_index += 1

                else:

                    f.write(mb_read_data(sock, config, 3, offset, (int(config["file_start_reg"]) + weight) - offset))
                    sys.stdout.write('=] 100%\n')

            f.close()

        else:
            print("Same file exist!", "Weight:", os.path.getsize(programm_path() + config["file_folder"] + "/" +str(select)), ":", weight*2)
        return 0
...
def mb_get_all_files(config):
    ...
    try:
        ...
        counter = int.from_bytes(mb_read_data(sock, config, 3, int(config["file_counter_reg"]), 1), byteorder='big')

        print("Total files: ", counter)

        for file_num in range(1, counter + 1):
            print("File number: ", file_num)

            while file_num != int.from_bytes(mb_read_data(sock, config, 3, int(config["file_select_reg"]), 1), byteorder='big'):
                mb_write_data(sock, config, 16, int(config["file_select_reg"]), 1, file_num)

            mb_get_file(sock, config)
